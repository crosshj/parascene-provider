...
const LONG_EDGE_DEFAULT = 1024;

/**
 * Target pixel size for a ratio with a fixed long edge (e.g. 9:16 → 576×1024).
 * @param {string} ratioKey
 * @param {number} [longEdge]
 * @returns {{ width: number, height: number }}
 */
export function dimensionsForAspectRatioLongEdge(ratioKey, longEdge = LONG_EDGE_DEFAULT) {
	const edge = Number(longEdge);
	const safeEdge = Number.isFinite(edge) && edge > 0 ? Math.round(edge) : LONG_EDGE_DEFAULT;
	const { width: rw, height: rh } = parseAspectRatioKey(ratioKey);
	if (rw >= rh) {
		return { width: safeEdge, height: Math.round((safeEdge * rh) / rw) };
	}
	return { width: Math.round((safeEdge * rw) / rh), height: safeEdge };
}
